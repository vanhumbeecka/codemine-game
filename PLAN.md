...
2. [~] **Real art**: sprite atlas — shaft walls, wall lanterns, and the parallax cave
   background render from the Kauzz "Pixel Valley | Cave" sheets (credited in README.md),
   defined in `src/sprites.js`. Elevator sprites, ore crystal sprites, and the DK
   Productions miner character were tried and reverted — they looked ugly next to the
   rest. Elevator/dwarf and ore veins are procedural Graphics again; better-matching
   art still wanted.
3. [ ] **More life**: background dwarves mining at ore veins (first attempt reverted with
   the miner sprites — needs art that fits the scene).
